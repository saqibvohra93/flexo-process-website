...
export const QuoteModal: React.FC<QuoteModalProps> = ({ isOpen, onClose, defaultTopic }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 dark:bg-navy-950/85 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-white/20 rounded-2xl max-w-xl w-full p-6 sm:p-8 shadow-2xl relative max-h-[95vh] overflow-y-auto">
        ...
        {submitted ? (
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-navy-950 border border-slate-200 dark:border-white/15 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-amber-500 dark:focus:border-flexo-yellow focus:outline-none"
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Substrate</label>
                  <select
                    value={formData.substrate}
                    onChange={e => setFormData({ ...formData, substrate: e.target.value })}
                    className="w-full bg-slate-50 dark:bg-navy-950 border border-slate-200 dark:border-white/15 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-white focus:border-amber-500 dark:focus:border-flexo-yellow focus:outline-none"
                  >
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Turnaround Urgency</label>
                  <select
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Project Details / Specs</label>
                <textarea
                  rows={3}
