...
import { Layers, Sparkles, Maximize2, Headphones, CheckCircle2, ArrowRight, Wrench, FileCheck, X } from 'lucide-react';
...
export const ServicesSection: React.FC<ServicesSectionProps> = ({ onSelectServiceForQuote }) => {
  ...
  return (
    <section id="services" className="py-16 lg:py-20 bg-slate-100/60 dark:bg-navy-900/50 border-t border-slate-200 dark:border-white/10 relative transition-colors duration-200">
      <div className="container-x relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-7">
          {SERVICES.map((srv) => (
            <div
              key={srv.id}
              className="group rounded-2xl bg-white dark:bg-navy-950/80 border border-slate-200 dark:border-white/10 hover:border-amber-500/50 dark:hover:border-flexo-yellow/50 p-6 sm:p-8 transition-all duration-300 flex flex-col justify-between shadow-sm dark:shadow-none hover:shadow-lg dark:hover:shadow-[0_12px_35px_rgba(0,0,0,0.5)] backdrop-blur-sm"
            >
              <div>
                {/* Icon & Equipment Tag */}
                <div className="flex items-center justify-between mb-5">
                  <div className="w-12 h-12 rounded-xl bg-slate-50 dark:bg-navy-900 border border-slate-200 dark:border-white/15 flex items-center justify-center group-hover:border-amber-500 dark:group-hover:border-flexo-yellow transition-colors shadow-inner">
                    {getIcon(srv.icon)}
                  </div>
                  <span className="text-[11px] font-mono font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-white/[0.04] px-3 py-1 rounded-full border border-slate-200 dark:border-white/10">
                    Industrial Standard
                  </span>
                </div>

                <h3 className="font-display font-bold text-2xl text-slate-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-flexo-yellow transition-colors mb-1.5">
                  {srv.title}
                </h3>
                <p className="text-xs font-mono text-amber-600 dark:text-flexo-yellow mb-4">
                  {srv.subtitle}
                </p>
                ...
                {/* Key Features List */}
                <div className="space-y-2 mb-6 text-xs text-slate-700 dark:text-slate-200">
              ...
              {/* Bottom Card Footer */}
              <div className="pt-5 border-t border-slate-200 dark:border-white/10 flex items-center justify-between text-xs">
                <button
                  onClick={() => setActiveServiceModal(srv)}
                  className="font-bold text-slate-700 dark:text-slate-300 hover:text-amber-600 dark:hover:text-flexo-yellow flex items-center gap-1.5 transition-colors"
                >
                  <FileCheck className="w-4 h-4 text-amber-500 dark:text-flexo-yellow" />
      ...
      {activeServiceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 dark:bg-navy-950/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-white/20 rounded-2xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            {/* Equipment Utilized */}
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-navy-950 border border-slate-200 dark:border-white/10 mb-6 text-xs">
              <span className="text-slate-500 dark:text-slate-400 block text-[11px] font-mono uppercase mb-1">
                Technology &amp; Equipment Deployed:
              </span>
              <span className="text-slate-900 dark:text-white font-semibold">{activeServiceModal.equipment}</span>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-700 dark:text-slate-300">
                {activeServiceModal.features.map((f, i) => (
                  <div key={i} className="flex items-start gap-2 bg-slate-50 dark:bg-white/[0.02] p-2 rounded-lg border border-slate-200 dark:border-white/5">
            ...
            <div className="mb-6 p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs">
              <h4 className="font-bold text-amber-800 dark:text-amber-300 uppercase tracking-wider mb-2">
